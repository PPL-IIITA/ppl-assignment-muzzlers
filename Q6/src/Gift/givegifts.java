...
public class givegifts {
    public static void GiveGift( List<Boy> boys,List<Gift> gifts,int i){
        ...
        if( b.is_Bcommitted(b)){
            for(Gift g: gifts){
                ...
                if((b.get_Bbudget()-b.get_expense())>g.get_value()){
                    ...
                    if(b.get_Bhappiness()+gf.get_Ghappiness()<i)
                    {
                        //System.out.print("in ifff...");
                        String FILENAM = "log.txt";
                        try (BufferedWriter bqwe = new BufferedWriter(new FileWriter(FILENAM,true))) {
                            String content =b.get_Bname()+" broke with "+gf.getGname();
                            b.Bclear();
                            bqwe.write(content);
                            bqwe.newLine();
                            
                            int numbf=ThreadLocalRandom.current().nextInt(1, 100);
                            content =gf.getGname()+" got newly commited with B "+numbf;
                            bqwe.write(content);
                            bqwe.newLine();
                        }
                        catch (IOException e) {
